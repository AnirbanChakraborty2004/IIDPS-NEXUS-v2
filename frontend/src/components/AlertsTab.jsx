...
const computeAttackDist = (alerts) => {
  const typeColors = {
    'DoS':        { color: '#ff003c', shadow: '#ff003c' },
    'Port Scan':  { color: '#ff9d00', shadow: '#ff9d00' },
    'Brute Force':{ color: '#b000ff', shadow: '#b000ff' },
    'DDoS':       { color: '#00f0ff', shadow: '#00f0ff' },
    'AI Triggered Attack Simulation': { color: '#ff003c', shadow: '#ff003c' },
  };
  const defaultStyle = { color: '#6366f1', shadow: '#6366f1' };
  const counts = {};
  ...
  return Object.entries(counts).map(([label, count]) => ({
    ...
    ...(typeColors[label] || defaultStyle),
  }));
};

/* Helper: extract blocked IPs from alerts */
const extractBlocked = (alerts) => {
  const blocked = [];
  const seen = new Set();
  alerts.forEach(a => {
    if (a.action === 'Blocked' && a.ip && !seen.has(a.ip)) {
      seen.add(a.ip);
      blocked.push({
        ip: a.ip,
        time: a.time,
...
const AlertsTab = ({ alerts = [], stats = {} }) => {
  const [selectedAlert, setSelectedAlert] = useState(null);

  const attackDist = computeAttackDist(alerts);
  const blockedIPs = extractBlocked(alerts);
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="relative flex-shrink-0" style={{ width:200, height:200 }}>
            <svg viewBox="0 0 200 200" width="200" height="200">
              ...
              <text x="100" y="112" textAnchor="middle" fill="#e2e8f0" fontSize="18" fontFamily="monospace" fontWeight="800">{totalAlerts}</text>
            </svg>
          </div>

          {/* Legend + bar breakdown */}
          <div className="flex-1 w-full flex flex-col gap-3">
            {attackDist.length > 0 ? attackDist.map((d) => (
              <div key={d.label}>
              ...
              <div className="text-gray-500 text-sm text-center py-4">No attack data available</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel">
        <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-2">
          ...
          <span className="text-xs text-gray-500 font-mono">Last {Math.min(alerts.length, 20)} Threats</span>
        </div>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs opacity-80 font-mono mt-1">{alert.ip} &rarr; {alert.target || 'Local System'}</div>
                </div>
              </div>
              <div className="flex items-center gap-4 text-xs font-mono opacity-80">
                <span>{formatTime(alert.time)}</span>
                <span className="px-2 py-1 bg-black/40 rounded border border-current">{alert.action || 'Logged'}</span>
              </div>
            </div>
          )) : (
      ...
      {selectedAlert && (
        <div className="glass-panel border-purple-500/30 bg-purple-900/10 relative">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm font-mono text-gray-300">
            ...
            <div><span className="text-gray-500">Target:</span><br/>{selectedAlert.target || 'Local System'}</div>
            ...
            <div><span className="text-gray-500">Action:</span><br/><span className="text-emerald-400">{selectedAlert.action || 'Logged'}</span></div>
          </div>
      ...
      <div className="glass-panel">
        <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-2">
          ...
          <span className="text-xs text-gray-500 font-mono">Total Blocked: {blockedIPs.length}</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {blockedIPs.length > 0 ? blockedIPs.map((block, idx) => (
            <div key={idx} className="flex justify-between items-center bg-black/40 p-3 rounded-lg border border-red-500/20">
              <div>
                ...
                <div className="text-xs text-gray-500">{formatTime(block.time)}</div>
              </div>
            ...
            <div className="col-span-full text-center text-gray-500 py-4 text-sm">No blocked IPs</div>
          )}
        </div>
      </div>
